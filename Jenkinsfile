...
//      https://github.com/im-sohel/events-app-api-server.git
//	    main
//      web-server-image
//      the following values can be found in the yaml:
//      demo_ui
//      demo_ui (name of the container to be replaced - in the template/spec section of the deployment)
...
pipeline {
   ...
   environment {
        ...
        imageName = 'sohelmalik007/api-server-image'
        dockerImage = ''
        }
    stages {
        stage('Get Source') {
            ...
            steps {
                ...
                git branch: 'main',
                    url: 'https://github.com/im-sohel/events-app-api-server.git'
        ...
        stage('Deploy Image') {
         ...
         stage('get k8s credentials') {
            ...
            steps {
                ...
                sh 'gcloud container clusters get-credentials events-app-cluster --zone us-central1-c --project roidtc-july2022-u417'
            }
        }     
         stage('update k8s') {
             agent {
            ...
            steps {
                echo 'Set the image'
                     sh "kubectl --kubeconfig=${WORKSPACE}/kube/.kube/config set image deployment/demo_api demo_api=${env.imageName}:${env.BUILD_NUMBER}"
            }
        }     
